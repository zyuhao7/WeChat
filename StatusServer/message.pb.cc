...
SendChatMsgRsp::SendChatMsgRsp(const SendChatMsgRsp& from)
  : ::PROTOBUF_NAMESPACE_ID::Message() {
  _internal_metadata_.MergeFrom<::PROTOBUF_NAMESPACE_ID::UnknownFieldSet>(from._internal_metadata_);
  touid_.UnsafeSetDefault(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited());
  if (!from._internal_touid().empty()) {
    touid_.Set(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), from._internal_touid(),
      GetArena());
  }
  ::memcpy(&error_, &from.error_,
    static_cast<size_t>(reinterpret_cast<char*>(&fromuid_) -
    reinterpret_cast<char*>(&error_)) + sizeof(fromuid_));
...
void SendChatMsgRsp::SharedDtor() {
  GOOGLE_DCHECK(GetArena() == nullptr);
  touid_.DestroyNoArena(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited());
}
...
#define CHK_(x) if (PROTOBUF_PREDICT_FALSE(!(x))) goto failure
  ...
  while (!ctx->Done(&ptr)) {
    ...
    switch (tag >> 3) {
      ...
      case 3:
        if (PROTOBUF_PREDICT_TRUE(static_cast<::PROTOBUF_NAMESPACE_ID::uint8>(tag) == 26)) {
          auto str = _internal_mutable_touid();
          ptr = ::PROTOBUF_NAMESPACE_ID::internal::InlineGreedyStringParser(str, ptr, ctx);
          CHK_(::PROTOBUF_NAMESPACE_ID::internal::VerifyUTF8(str, "message.SendChatMsgRsp.touid"));
          CHK_(ptr);
        } else goto handle_unusual;
        continue;
      default: {
...
::PROTOBUF_NAMESPACE_ID::uint8* SendChatMsgRsp::_InternalSerialize(
  ...
  // string touid = 3;
  if (this->touid().size() > 0) {
    ::PROTOBUF_NAMESPACE_ID::internal::WireFormatLite::VerifyUtf8String(
      this->_internal_touid().data(), static_cast<int>(this->_internal_touid().length()),
      ::PROTOBUF_NAMESPACE_ID::internal::WireFormatLite::SERIALIZE,
      "message.SendChatMsgRsp.touid");
    target = stream->WriteStringMaybeAliased(
        3, this->_internal_touid(), target);
  }
...
void SendChatMsgRsp::InternalSwap(SendChatMsgRsp* other) {
  using std::swap;
  _internal_metadata_.Swap<::PROTOBUF_NAMESPACE_ID::UnknownFieldSet>(&other->_internal_metadata_);
  touid_.Swap(&other->touid_, &::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), GetArena());
  ::PROTOBUF_NAMESPACE_ID::internal::memswap<
      PROTOBUF_FIELD_OFFSET(SendChatMsgRsp, fromuid_)
      + sizeof(SendChatMsgRsp::fromuid_)
